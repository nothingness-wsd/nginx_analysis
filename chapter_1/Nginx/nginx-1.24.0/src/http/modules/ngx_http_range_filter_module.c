...
/*
 HTTP Range 处理模块中的头部过滤器（header filter），负责处理客户端发送的 Range 请求头，
 并根据是否满足条件决定是否返回部分内容（206 Partial Content）或告知客户端服务器支持字节范围请求（通过 Accept-Ranges: bytes 响应头）
*/
static ngx_int_t
ngx_http_range_header_filter(ngx_http_request_t *r)
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_range_header_filter\n\n");
...
*/
    ...
    {
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "return ngx_http_next_header_filter(r);\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_range_body_filter\n\n");
	...
	// 单段 Range 处理
	// 如果只有一个 Range 区间，调用 ngx_http_range_singlepart_body 进行高效处理（只截取该区间数据）
    if (ctx->ranges.nelts == 1) {
...
{
	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    dprintf(trace_file_fd,"function:\t" "ngx_http_range_header_filter_init\n\n");
...
{
	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    dprintf(trace_file_fd,"function:\t" "ngx_http_range_body_filter_init\n\n");
