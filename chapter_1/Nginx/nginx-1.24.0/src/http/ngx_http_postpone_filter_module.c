...
/*
ngx_http_postpone_filter_module 是 Nginx HTTP 模块系统中的一个输出过滤器模块（output filter module），它的核心作用是：
处理响应体（response body）中多个子请求（subrequests）的输出顺序问题，确保子请求的输出内容按照它们在主响应中“被发起”的逻辑顺序正确排列。 
*/
...
ngx_http_postpone_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
	...
	dprintf(trace_file_fd, "function:\t" "ngx_http_postpone_filter\n\n");
	...
	// 检查是否是内存中的子请求，如果是则调用专门的内存处理函数
    if (r->subrequest_in_memory) 
    {
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (r->subrequest_in_memory)\nreturn ngx_http_postpone_filter_in_memory(r, in);\n\n");
    	
        return ngx_http_postpone_filter_in_memory(r, in);
    }

	// 检查当前请求是否不是活动请求（c->data指向当前活动请求）
    if (r != c->data) 
    {
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    ...
    {
		dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
		dprintf(trace_file_fd, "if (r->postponed == NULL)\n\n");
		
		// 如果有输入数据或连接还在缓冲数据，则调用下一个body filter处理
        if (in || c->buffered) 
        {
			dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    		dprintf(trace_file_fd, "if (in || c->buffered)\n" "return ngx_http_next_body_filter(r->main, in);\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_postpone_filter_init\n\n");
