...
// 它负责初始化请求上下文，并决定从哪个阶段（phase）开始执行 HTTP 请求处理引擎
void
ngx_http_handler(ngx_http_request_t *r)
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_handler\n\n");
    ...
    if (!r->internal) {
    	...
    	//处理内部请求（r->internal == 1）的分支
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "else\tr->internal\n\n");
    	
    	// 获取 HTTP 核心模块的 main 级别配置
        cmcf = ngx_http_get_module_main_conf(r, ngx_http_core_module);
        
        //将 phase_handler 设置为 server rewrite 阶段的起始索引
        r->phase_handler = cmcf->phase_engine.server_rewrite_index;
        
		dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        dprintf(trace_file_fd, "r->phase_handler=%ld\n\n", r->phase_handler);
    }
	...
	// 设置写事件处理器为 ngx_http_core_run_phases
	// 当连接可写时（例如 socket 缓冲区有空间），会调用 write_event_handler
    r->write_event_handler = ngx_http_core_run_phases;
    
    //立即启动 HTTP 请求处理阶段引擎。
	//ngx_http_core_run_phases 会从 r->phase_handler 指定的阶段开始，依次调用注册在该阶段的所有 handler
    ngx_http_core_run_phases(r);
    
    dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    dprintf(trace_file_fd, "ngx_http_core_run_phases(r) completed\n\n");
    
    dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    dprintf(trace_file_fd, "function ngx_http_handler:\treturn\n\n");
}
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_core_run_phases\n\n");
    ...
    while (ph[r->phase_handler].checker) {
		...
		dprintf(trace_file_fd, "rc(%ld) = ph[r->phase_handler].checker\n\n", rc);
		
        if (rc == NGX_OK) {
        	
        	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        	dprintf(trace_file_fd, "if (rc == NGX_OK) {\nfunction ngx_http_core_run_phases:\treturn;\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_core_rewrite_phase\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_core_find_config_phase\n\n");
	...
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "%ld = ngx_http_core_find_location(r);\n\n", rc);
	...
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "ngx_http_update_location_config(r);\n\n");
    ...
    {
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (r->headers_in.content_length_n != -1\n\n");
    ...
    if (rc == NGX_DONE) {
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (rc == NGX_DONE)\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_core_post_rewrite_phase\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" "function:\t" "ngx_http_core_access_phase\n\n", __FILE__, __LINE__);
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_core_post_access_phase\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_core_content_phase\n\n");
    ...
    if (rc != NGX_DECLINED) {
    	...
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (rc != NGX_DECLINED) {\nngx_http_finalize_request(r, rc);\n\n");
    	
        ngx_http_finalize_request(r, rc);
        
        dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        dprintf(trace_file_fd, "ngx_http_finalize_request(r, rc) completed\n\n");
        
        dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        dprintf(trace_file_fd, "function ngx_http_core_content_phase:\treturn NGX_OK;\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_update_location_config\n\n");
    ...
    if (r->method & clcf->limit_except) {
    	...
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (r->method & clcf->limit_except)\n\n");
    ...
    if ((ngx_io.flags & NGX_IO_SENDFILE) && clcf->sendfile) {
		...
		dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if ((ngx_io.flags & NGX_IO_SENDFILE) && clcf->sendfile)\n");
    	dprintf(trace_file_fd, "r->connection->sendfile = 1;\n\n");
    	...
    	dprintf(trace_file_fd, "r->connection->sendfile = 0;\n\n");
    ...
    if (clcf->client_body_in_file_only) {
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (clcf->client_body_in_file_only)\n\n");
    	...
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "else\n");
    	dprintf(trace_file_fd, "r->request_body_file_log_level = NGX_LOG_WARN;\n\n");
	...
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "r->request_body_in_single_buf=%d\n\n", r->request_body_in_single_buf);

    if (r->keepalive) {
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (r->keepalive)\n\n");
    	
        if (clcf->keepalive_timeout == 0) {
        	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        	dprintf(trace_file_fd, "if (clcf->keepalive_timeout == 0)\n\n");
        	
            r->keepalive = 0;

        } else if (r->connection->requests >= clcf->keepalive_requests) {
        	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        	dprintf(trace_file_fd, "else if (r->connection->requests >= clcf->keepalive_requests)\n\n");
        ...
        {
        	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        	dprintf(trace_file_fd, "else if (ngx_current_msec - r->connection->start_time > clcf->keepalive_time\n\n");
        ...
        {
        	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        	dprintf(trace_file_fd, "else if (r->headers_in.msie6\n\n");
        ...
        {
        	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        	dprintf(trace_file_fd, "else if (r->headers_in.safari\n\n");
    ...
    if (!clcf->tcp_nopush) {
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (!clcf->tcp_nopush)\n\n");
    ...
    if (clcf->handler) {
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (clcf->handler)\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_core_find_location\n\n");
    ...
    if (rc == NGX_AGAIN) {
    	...
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (rc == NGX_AGAIN)\n\n");
    ...
    if (rc == NGX_OK || rc == NGX_DONE) {
    	...
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (rc == NGX_OK || rc == NGX_DONE)\n\n");
    ...
    if (noregex == 0 && pclcf->regex_locations) {
    	...
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (noregex == 0 && pclcf->regex_locations)\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_core_find_static_location\n\n");
    ...
    n = write(trace_file_fd, r->uri.data, r->uri.len);
    ...
    for ( ;; ) {
		...
		dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
		dprintf(trace_file_fd, "node->len=%d\n", node->len);
...
/*
用于根据请求的文件扩展名（r->exten）设置 HTTP 响应头中的 Content-Type 字段

MIME 类型（Multipurpose Internet Mail Extensions，多用途互联网邮件扩展）最初是为电子邮件系统设计的，用于标识邮件中附件的内容格式。
后来被广泛应用于 HTTP 协议 中，用来告诉浏览器（或其他客户端）服务器返回的内容是什么类型的，以便客户端能正确地解析、渲染或处理这些内容
...
.txt	text/plain						纯文本（无匹配）	
		application/octet-stream 		通用二进制流（默认下载）
		
浏览器看到 text/html，就知道应该用 HTML 渲染引擎解析。
如果是 image/png，浏览器会尝试显示图片。
如果是 application/json，前端 JavaScript 可以用 fetch().json() 直接解析。
如果是 application/octet-stream，浏览器通常会触发下载，而不是显示内容。		
*/
ngx_int_t
ngx_http_set_content_type(ngx_http_request_t *r)
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_set_content_type\n\n");
    ...
    if (r->exten.len) {
		...
		// 作用：遍历扩展名的每一个字符。
		// 意义：逐字符处理，用于计算哈希或检测是否包含大写字母。
        for (i = 0; i < r->exten.len; i++) {
            c = r->exten.data[i];

			/*
			作用：判断当前字符是否为大写字母（ASCII 范围 'A' 到 'Z'）。
			逻辑：如果发现大写字母，说明扩展名不是全小写。
			意义：Nginx 的 MIME 类型哈希表是基于小写扩展名构建的，因此必须将扩展名转为小写才能正确匹配。
			*/
            if (c >= 'A' && c <= 'Z') {
...
*/
		dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
		dprintf(trace_file_fd, "ngx_hash_find\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_set_exten\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_set_etag\n\n");
...
/*
检查配置项 clcf->etag 是否为 0（即是否禁用了 ETag）。
如果用户在配置文件中设置了 etag off;，则 clcf->etag 为 0，此时不生成 ETag，直接返回 NGX_OK（成功，但不设置头）。
*/
    if (!clcf->etag) {
    
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (!clcf->etag)\n\n");
    	
        return NGX_OK;
    }

/*
调用 ngx_list_push 向响应头列表 r->headers_out.headers 中添加一个新的头部字段结构体。
ngx_list_push 返回指向新分配的 ngx_table_elt_t 结构体的指针，用于后续填充 ETag 的 key 和 value。
headers_out.headers 是一个 ngx_list_t 类型的链表，用于存储所有要发送的 HTTP 响应头。
*/
...
分配大小为：
NGX_OFF_T_LEN：表示 off_t 类型（通常为文件大小，即 content_length_n）最大十进制位数。
NGX_TIME_T_LEN：表示 time_t 类型（即 last_modified_time）最大十六进制位数（注意：这里用的是 %xT，是十六进制）。
+3：用于两个引号 "..." 和一个连字符 '-'，即格式为 ""%xT-%xO""，共需 2（引号）+ 1（连字符）= 3 字节。
举例：若 last_modified_time=0x6789abcd，content_length_n=0x12345，则 ETag 为 ""6789abcd-12345""。
*/
...
/*
使用 ngx_sprintf 格式化 ETag 值到刚分配的内存中。
格式字符串： ""%xT-%xO""
%xT：Nginx 特有的格式说明符，表示将 time_t 类型（last_modified_time）以十六进制输出。
%xO：表示将 off_t 类型（content_length_n）以十六进制输出。
ngx_sprintf 返回写入结束后的指针，减去起始地址 etag->value.data，得到实际写入的字节数，赋值给 etag->value.len。
最终 ETag 值形如："a1b2c3d4-12345"（带双引号）。
注意：ETag 标准要求值必须用双引号括起来，所以格式字符串中包含 "。 
*/
...
{
    dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    dprintf(trace_file_fd,"function:\t" "ngx_http_send_header\n\n");
    
    // 检查是否设置了 post_action 标志
    if (r->post_action) {
    
    	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd,"if (r->post_action)\nreturn NGX_OK;\n\n");
...
/*
作用：调用 header filter 链的入口，开始实际发送响应头。
逻辑详解：
nginx 使用“过滤器链（filter chain）”机制来处理响应头和响应体。
ngx_http_top_header_filter 是 header filter 链的最顶层函数指针，通常指向 ngx_http_header_filter（但可能被模块如 gzip、SSI 等 hook 修改）。
调用它会依次执行所有注册的 header filter 模块，最终由 ngx_http_header_filter 负责：
序列化 HTTP 响应头（状态行 + 所有 headers_out 中的字段）。
将响应头写入输出缓冲区。
调用底层网络发送函数（如 writev）将数据发送给客户端。
设置 r->header_sent = 1。
意义：这是真正触发“发送响应头”动作的地方，前面的代码只是做前置检查和准备。
*/
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "return ngx_http_top_header_filter(r);\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_output_filter\n\n");
	...
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "rc = ngx_http_top_body_filter(r, in);\n\n");
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_map_uri_to_path\n\n");
	...
	/*
	 检查 alias 与 URI 重写冲突
	如果配置了 alias，但当前请求的 URI 是被重写过的（例如通过 rewrite 指令），且 location 不再“有效”（r->valid_location == 0），则禁止使用 alias。
	原因：alias 依赖原始 URI 的结构来替换路径。如果 URI 被重写，原始结构丢失，alias 行为不可预测。
	r->valid_location 在 URI 重写后会被设为 0，表示当前 location 可能不再适用。
	此时记录 ALERT 级别日志 并返回 NULL，表示路径映射失败。
	*/
    if (alias && !r->valid_location) {
    	
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (alias && !r->valid_location)\n\n");
    ...
    if (clcf->root_lengths == NULL) {
        ...
        *root_length = clcf->root.len;
		// 计算总路径长度
		// 总长度 = root 长度 + 预留空间 + URI 长度（减去 alias 偏移）+ 1（用于结尾 \0）。
        path->len = clcf->root.len + reserved + r->uri.len - alias + 1;
        
        dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        dprintf(trace_file_fd, 	"path->len(%ld) = clcf->root.len(%ld) + reserved(%ld) + r->uri.len(%ld) - alias(%ld) + 1;\n\n",
        						 path->len, clcf->root.len, reserved, r->uri.len, alias);
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_internal_redirect\n\n");
	...
	// 重新解析文件扩展名
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "ngx_http_set_exten(r);\n\n");
	...
	//ngx_http_max_module 是编译时确定的模块总数。
	...
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "ngx_http_handler(r);\n\n");
	
    ngx_http_handler(r);

	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "ngx_http_internal_redirect return NGX_DONE;\n\n");
...
//作用是根据配置项 disable_symlinks 和 disable_symlinks_from 设置当前请求所要访问的文件路径是否启用符号链接（symlink）限制。
//该功能主要用于防止通过符号链接访问到本不应被访问的文件（例如绕过 root 或 alias 限制）
//返回类型：ngx_int_t（Nginx 的整型，通常用 NGX_OK、NGX_ERROR 等宏表示状态）
//参数：
//r：当前 HTTP 请求结构体，用于获取运行时上下文（如变量值）。
//clcf：当前 location 的核心配置（ngx_http_core_loc_conf_t），包含 disable_symlinks 和 disable_symlinks_from 配置。
//path：要访问的文件路径（绝对路径）。
//of：文件打开信息结构体（ngx_open_file_info_t），用于传递最终的符号链接策略给后续文件打开逻辑。
ngx_int_t
ngx_http_set_disable_symlinks(ngx_http_request_t *r,
    ngx_http_core_loc_conf_t *clcf, ngx_str_t *path, ngx_open_file_info_t *of)
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_http_set_disable_symlinks\n\n");
	...
	//检查是否配置了 disable_symlinks_from：
	//如果用户没有设置 disable_symlinks_from 指令（即该字段为 NULL），则无需进一步处理，直接返回成功。
    if (clcf->disable_symlinks_from == NULL) {
    
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd, "if (clcf->disable_symlinks_from == NULL)\n");
    	dprintf(trace_file_fd, "return NGX_OK;\n\n");
...
ngx_http_core_preconfiguration(ngx_conf_t *cf)
{
	...
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "ngx_http_variables_add_core_vars\n\n");
	
	rc = ngx_http_variables_add_core_vars(cf);
...
{
	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    dprintf(trace_file_fd,"function:\t" "ngx_http_core_merge_loc_conf\n\n");
    ...
    if (prev->types && prev->types_hash.buckets == NULL) {
		...
		dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd,"if (prev->types && prev->types_hash.buckets == NULL)\n\n");
    ...
    if (conf->types == NULL) {
		...
		dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd,"if (conf->types == NULL)\n\n");
    ...
    if (conf->types_hash.buckets == NULL) {
		...
		dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd,"if (conf->types_hash.buckets == NULL)\n\n");
