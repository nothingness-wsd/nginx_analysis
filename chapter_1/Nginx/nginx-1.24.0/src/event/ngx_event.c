...
/*
这个函数用于处理读事件，根据不同的事件模型（kqueue、epoll、select/poll等）来添加或删除读事件监听。
*/
ngx_int_t
ngx_handle_read_event(ngx_event_t *rev, ngx_uint_t flags)
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "function:\t" "ngx_handle_read_event\n\n");
	
	//检查当前使用的是边缘触发（ET）事件模型（kqueue、epoll）
	//NGX_USE_CLEAR_EVENT 表示使用边缘触发模式
    if (ngx_event_flags & NGX_USE_CLEAR_EVENT) {

		dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
		dprintf(trace_file_fd, "if (ngx_event_flags & NGX_USE_CLEAR_EVENT) {\n\n");
		
        /* kqueue, epoll */

		//如果读事件既没有激活（active为假）也没有准备就绪（ready为假）
		//active 表示事件是否在事件驱动机制中注册
		//ready 表示事件是否已经就绪
        if (!rev->active && !rev->ready) {
        	
        	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
        	dprintf(trace_file_fd, "if (!rev->active && !rev->ready) {\nngx_add_event\n\n");
        	
        	//调用 ngx_add_event 添加读事件到事件驱动机制中
		...
		dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
		dprintf(trace_file_fd, "function ngx_handle_read_event:\treturn NGX_OK;\n\n");
...
ngx_events_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	...
	dprintf(trace_file_fd, "ngx_count_modules\n\n");
	
    ngx_event_max_module = ngx_count_modules(cf->cycle, NGX_EVENT_MODULE);

	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd, "统计 event 类型模块的数量并分配索引 完成\n");
	dprintf(trace_file_fd, "ngx_count_modules completed\n\n");
...
ngx_event_connections(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ecf->connections == (ngx_uint_t) NGX_ERROR) {
		...
		dprintf(trace_file_fd, "转换指令参数 失败\n"
								"function %s:\t" "return NGX_CONF_ERROR\n\n", __func__);
