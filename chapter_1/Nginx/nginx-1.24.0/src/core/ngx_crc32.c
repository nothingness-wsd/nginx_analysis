...
ngx_crc32_table_init(void)
{
	...
	dprintf(trace_file_fd,"function:\t" "ngx_crc32_table_init\n\n");
	
    void  *p;

	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd,"检查 ngx_crc32_table_short 是否已经对齐到缓存行边界\n\n");
    ...
    {
    	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    	dprintf(trace_file_fd,"已经对齐，无需再做任何操作，直接返回成功\n");
    	dprintf(trace_file_fd,"function ngx_crc32_table_init:\t" "return NGX_OK\n\n");
    	
        return NGX_OK;
    }
    
	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd,"未对齐\n\n");
	...
	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd,"内存对齐 ngx_cacheline_size\n\n");
	
    p = ngx_align_ptr(p, ngx_cacheline_size);
	
	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd,"复制 ngx_crc32_table16\n\n");
