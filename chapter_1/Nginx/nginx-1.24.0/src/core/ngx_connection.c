...
ngx_open_listening_sockets(ngx_cycle_t *cycle)
{
	...
	dprintf(trace_file_fd, "function:\t" "ngx_open_listening_sockets\n\n" );
...
ngx_configure_listening_sockets(ngx_cycle_t *cycle)
{
	...
	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__ );
    dprintf(trace_file_fd,"function:\t"  "ngx_configure_listening_sockets\n\n");
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ls[i].add_deferred || ls[i].delete_deferred) {
			...
			dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__ );
			dprintf(trace_file_fd, "setsockopt(TCP_DEFER_ACCEPT)\n\n");
...
/*
它的作用是将一个连接（connection）标记为“可重用”或“不可重用”，
并相应地将其加入或移出“可重用连接队列”，
主要用于管理处于空闲但尚未关闭的连接（例如 HTTP keep-alive 连接），
以便高效复用这些连接，提高服务器性能。
*/
void
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__ );
	dprintf(trace_file_fd, "function:\t" "ngx_reusable_connection\n\n");
    ...
    dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__ );
    dprintf(trace_file_fd, "c->reusable = reusable(%ld);\n\n", reusable);
    ...
    if (reusable) {	
        ...
        dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__ );
        dprintf(trace_file_fd, "ngx_cycle->reusable_connections_n=%ld\n\n", ngx_cycle->reusable_connections_n);
...
{
	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__ );
	dprintf(trace_file_fd, "function:\t" "ngx_tcp_nodelay\n\n");
    ...
    if (c->tcp_nodelay != NGX_TCP_NODELAY_UNSET) {
    	...
    	dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__ );
    	dprintf(trace_file_fd, "if (c->tcp_nodelay != NGX_TCP_NODELAY_UNSET)\n\n");
...
#if (NGX_SOLARIS)
		dprintf(trace_file_fd, "FILE=%s:LINE=%d\n" , __FILE__, __LINE__ );
		dprintf(trace_file_fd, "setsockopt() == -1\n\n");
