...
// 用于在 Nginx 启动时初始化 slab 内存分配器中与内存块大小相关的关键常量。
// Slab 分配器是 Nginx 自己实现的一种高效的小内存分配机制，主要用于共享内存（如用于缓存、共享内存字典等）中的内存管理
void
ngx_slab_sizes_init(void)
{
	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd,"function:\t" "ngx_slab_sizes_init\n\n");
	...
	// ngx_slab_exact_size 是“exact”类的块大小。
    ...
    // 计算 ngx_slab_exact_shift，即 log2(ngx_slab_exact_size)
    for (n = ngx_slab_exact_size; n >>= 1; ngx_slab_exact_shift++) {
        /* void */
    }
    
    dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
    dprintf(trace_file_fd,"ngx_slab_max_size=%ld\n", ngx_slab_max_size);
    dprintf(trace_file_fd, "ngx_slab_exact_size=%ld\n", ngx_slab_exact_size);
    dprintf(trace_file_fd, "ngx_slab_exact_shift=%ld\n\n", ngx_slab_exact_shift);
    dprintf(trace_file_fd, "function ngx_slab_sizes_init:\t" "return\n\n");
}
...
{
	dprintf(trace_file_fd,"FILE=%s:LINE=%d\n" , __FILE__, __LINE__);
	dprintf(trace_file_fd,"function:\t" "ngx_slab_init\n\n");
